/**
 * Readers that turn a component's stored parameters into render-ready values.
 *
 * Pure and server-safe: the admin previews and the public renderer both read
 * these, which is what keeps a block looking the same in the builder as it does
 * on the site. Values are always partial (a document may be half-filled in the
 * form, or an older document may predate a field), so every accessor tolerates
 * missing input.
 */
...
import { resolveColorChoice } from './theme-css'
import type { ThemeTokenPath } from './theme-tokens'
...
/** Full section height, as the public site renders it. */
