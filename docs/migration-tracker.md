...
| Post page: article measure | ~886px (50% of the row less its padding), centred | 960px, left-aligned | Ours was **672px** (`max-w-[42rem]` on `PostBody`) inside a 1440px container — half the row empty, which is what read as broken. 960px fills it and shares a left edge with the title and author; the full 1440px would put ~180 characters on a line |
