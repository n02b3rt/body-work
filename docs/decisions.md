...
| Date | Decision | Why |
|---|---|---|
| 2026-09-17 | Hub's landing page is a real `src/app/[locale]/hub/` segment, invisibly rewritten from `/` by `proxy.ts` on its own host, not a `(hub)/` route group | Next.js refuses two route groups whose `page.tsx` resolve to the same path (`route-groups.md`: "Routes in different groups should not resolve to the same URL path... would... cause an error"), and Centrum keeps `/` (its ~15 existing routes, real SEO value). See [`sites.md`](./sites.md) |
