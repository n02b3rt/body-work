...
| 2026-08-03 | Performance is measured from the build in gzip (`pnpm check:perf`), Lighthouse only confirms | Lighthouse measures one URL through a browser over a network that is never twice the same; `scripts/perf-budget.mjs` is exact and covers all 141 pages. Budgets in `scripts/perf-budget.json` come down, never up. Full method and the localhost bias: [`performance.md`](./performance.md) |
| 2026-08-03 | Decorative icons stay files with `loading="lazy"`; no inline SVG sprite | React 19 promotes an eager image in the initial shell into a `<link rel="preload">` ahead of the LCP image, so the preloads had to go. A sprite removed them and cost 160 ms of document work and ~150 ms of TBT on a throttled mobile CPU. Trading network requests for main-thread work is the wrong direction on a phone |
| 2026-08-03 | Carousels on native `scroll-snap`, embla removed | Snapping, dragging and momentum are browser features; embla was ~7 KB gzip per page to reimplement them. The loop keeps the reference's Swiper behaviour by duplicating the track, and the copy is added after hydration so the server never renders it. `src/components/ui/use-scroll-carousel.ts` |
| 2026-08-03 | Compression stays with Next until Apache gets brotli, and then moves in one step | `compress` defaults to true, so Node hands the proxy an already-gzipped body that Apache cannot recompress. `compress: false` without the Apache module ships uncompressed HTML. Sequence: [`runbooks/apache-http2-brotli.md`](./runbooks/apache-http2-brotli.md) |
