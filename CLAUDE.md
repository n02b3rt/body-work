...
| Open Graph cards | `scripts/generate-og-images.mjs`, `public/images/og/` | 1200x630 **JPEG** cut from each page's hero (some scrapers still refuse WebP). Routes without one fall back to `og-default.jpg`; add a row to the script when a route starts declaring its own `image` |
